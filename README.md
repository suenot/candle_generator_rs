...
## Bulk vs Batch: в чём разница?

- **Bulk ingestion** — это быстрая загрузка большого массива трейдов (например, из файла или потока) без деления на партии, просто "залить всё" в агрегатор.
- **Batch processing/aggregation** — это обработка трейдов партиями (например, по файлам, по дням, по 1 млн штук), с прогрессом, логированием, контролем ошибок, поддержкой разных форматов и сценариев.

**В нашем проекте:**
- Bulk ingestion — это просто быстрая загрузка большого массива трейдов в память/агрегатор (см. examples/bulk_ingestion.rs).
- Batch aggregation — это обработка исторических данных по файлам/пакетам с прогрессом, флагами, контролем, поддержкой CSV, Parquet, DuckDB, QuestDB, ClickHouse и др.

## Новый пакет: candle_batch_aggregator

Для индустриальных сценариев пакетной обработки исторических данных реализован отдельный CLI-инструмент `candle_batch_aggregator` (см. папку candle_batch_aggregator/ в workspace):
- Поддержка всех популярных форматов (CSV, Parquet, DuckDB, QuestDB, ClickHouse и др.)
...
**Пример запуска:**
```bash
candle-batch-aggregator -i ./data -s BTCUSDT,ETHUSDT -t 1,5,15,60 -f csv -b -p
```

Подробнее — см. README.md внутри candle_batch_aggregator/ и примеры запуска.
